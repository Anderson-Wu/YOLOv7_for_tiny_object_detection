...
'''
class external_Concat(nn.Module):
    def __init__(self, dimension=1):
        super(external_Concat, self).__init__()
        self.d = dimension
...
class SCConv(nn.Module):
    def __init__(self, inplanes, planes,pooling_r=4, stride=1, padding=1, dilation=1, groups=1, norm_layer=nn.BatchNorm2d):
...
class SELayer(nn.Module):
    def __init__(self,c1,p=None, g=1, act=True,reduction=16):
    ...
    def forward(self,x):
        ...
        y = self.fclayer(y).view(b,c,1,1)
        return x*y.expand_as(x)




class SELayerGANet(nn.Module):
    def __init__(self,c1,p=None, g=1, act=True,reduction=16):
        super(SELayerGANet,self).__init__()
        self.avg_pool=nn.AdaptiveAvgPool2d(1)
        self.fclayer = nn.Sequential(
            nn.Linear(c1,c1//reduction,bias=False),
            nn.ReLU(inplace=True),
...
class Multiple(nn.Module):
    ...
    def forward(self,x):
        ...
        y = x[1]*x[0].expand_as(x[1])
        #print(y.shape)
        return y



def insertLayer(model):
    print(type(model.model))
    pass
    
    #for index,child in enumerate(model.named_modules()):
